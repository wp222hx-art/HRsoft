  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-2 text-sm text-slate-400">
        <Link href="/pro" className="hover:text-white"><ArrowLeft className="inline h-3 w-3" /> {t('modules.crumb.dashboard')}</Link>
        <span>/</span><span>{t('modules.crumb.module')}</span><span>/</span>
        <span className="text-white">{t(`mod.${params.slug}.cn`)}</span>
      </div>
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h1 className="text-2xl font-bold">{t(`mod.${params.slug}.cn`)}</h1>
              </div>
              {meta.killer && (
                <span className="nova-chip bg-gold-500/15 text-gold-400 ring-gold-500/40">
                  {t('modules.killerChipPro')}
                </span>
              )}
            </div>
            <p className="mt-3 text-slate-300">{t(`mod.${params.slug}.tagline`)}</p>
            <div className="mt-3 flex flex-wrap gap-3 text-xs">
              <div className="rounded-lg bg-white/5 px-3 py-1.5">
                <span className="text-slate-500">{t('modules.vsCopiLabel')}</span>
                <span className="ml-1 text-slate-300">{meta.copi}</span>
              </div>
              <div className="rounded-lg bg-nova-500/10 px-3 py-1.5">
                <span className="text-slate-400">{t('modules.superLabel')}</span>
